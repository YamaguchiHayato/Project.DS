	...
	public:
		...
		void RegistShaderResource(int registerNo, IShaderResource& sr)
		{
			RegistResource(
				...
				L"DescriptorHeap RegistShaderResource()レジスタ番号範囲Over"
			);
		}
		...
		void RegistUnorderAccessResource(int registerNo, IUnorderAccessResrouce& sr)
		{
			RegistResource(
				...
				L"RegistUnorderAccessResource()レジスタ番号が範囲範囲Over"
			);
		}
		/// <summary>
		...
		void RegistConstantBuffer(int registerNo, ConstantBuffer& cb)
		{
			RegistResource(
				...
				L"RegistConstantBufferレジスタ番号範囲over"
			);
		}
		/// <summary>
		...
		void RegistSamplerDesc(int registerNo, const D3D12_SAMPLER_DESC& desc)
		{
			RegistResource(
				...
				m_numSamplerDesc,
				MAX_SAMPLER_STATE,
				L"DescriptorHeap::RegistSamplerDesc()レジスタ番号が範囲Over"
			);
		}
		/// <summary>
		/// ディスクリプタヒープへの登録を確定。
