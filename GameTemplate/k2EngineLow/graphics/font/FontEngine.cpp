...
namespace nsK2EngineLow {
	...
	Vector2 FontEngine::MeasureString(const wchar_t* text, float scale) const
	{
		if (m_spriteFont == nullptr || text == nullptr) {
			return Vector2::Zero;
		}
		// SpriteFont::MeasureString はスケール1.0基準の(幅,高さ)を返すのでスケールを乗算する。
		DirectX::XMVECTOR size = m_spriteFont->MeasureString(text);
		return Vector2(DirectX::XMVectorGetX(size) * scale,
			DirectX::XMVectorGetY(size) * scale);
	}
}