...
namespace nsK2EngineLow {
	...
	public:
		...
		/// <summary>
		/// 頂点データの一部範囲だけを頂点バッファにコピー。
		/// モーフのように「一部の頂点しか変わらない」更新で全転送を避けるために使う。
		/// </summary>
		/// <param name="src">コピー元データ（範囲の先頭）。</param>
		/// <param name="offsetBytes">頂点バッファ内の書き込み開始位置（バイト）。</param>
		/// <param name="sizeBytes">コピーするサイズ（バイト）。</param>
		void CopyRange(const void* src, unsigned int offsetBytes, unsigned int sizeBytes);
		/// <summary>
		/// 頂点バッファビューを取得。
		/// </summary>
		/// <returns>ビュー</returns>
