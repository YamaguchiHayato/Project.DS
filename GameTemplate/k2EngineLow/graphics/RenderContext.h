...
namespace nsK2EngineLow {
	...
	public:
		...
		// =============================
		/// <summary>
		/// インデックス付きプリミティブを指定位置から描画（PMX用）
		/// </summary>
		/// <param name="indexCount">描画するインデックス数</param>
		/// <param name="startIndexLocation">開始インデックス位置</param>
		void DrawIndexed(UINT indexCount, UINT startIndexLocation)
		{
			m_commandList->DrawIndexedInstanced(indexCount, 1, startIndexLocation, 0, 0);
		}
		// =============================
		// PMX用追加要素ここまで！
		// =============================
		...
		/// <summary>
		/// インスタンシング描画（同じメッシュを instanceCount 個まとめて描く）。ペンライト等で使用。
		/// </summary>
