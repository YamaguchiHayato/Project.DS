...
namespace nsK2EngineLow {
	...
	void IndexBuffer::Release()
	{
		...
		m_indexBuffer = nullptr; // ★キューへ渡したら手放す（二重解放防止。Texture.cpp参照）
	}
	...
	void IndexBuffer::Copy(uint32_t* srcIndecies, int indexCount)
	{
		uint32_t* pData;
		m_indexBuffer->Map(0, nullptr, reinterpret_cast<void**>(&pData));
		memcpy(pData, srcIndecies, indexCount * sizeof(uint32_t));
		m_indexBuffer->Unmap(0, nullptr);
	}
