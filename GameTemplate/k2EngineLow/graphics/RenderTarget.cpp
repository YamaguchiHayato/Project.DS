	...
	)
	{
		...
		if (depthStencilFormat != DXGI_FORMAT_UNKNOWN) {
			if (!CreateDepthStencilTexture(*g_graphicsEngine, d3dDevice, w, h, depthStencilFormat)) {
				MessageBoxA(nullptr, "深度ステンシルバッファの作成に失敗しました。", "エラー", MB_OK);
				return false;
			}
		}
		...
		if (!CreateDescriptorHeap(*g_graphicsEngine, d3dDevice)) {
			MessageBoxA(nullptr, "ディスクリプタヒープの作成に失敗しました。", "エラー", MB_OK);
			return false;
		}
		...
		// ★ フォーマットを記録！
		m_colorBufferFormat = colorFormat;
		m_depthBufferFormat = depthStencilFormat;
