...
namespace nsK2EngineLow {
    ...
    // ==========================================
    // ★ここを追加：画像を切り抜いて頂点バッファを更新する
    // ==========================================
    void Sprite::SetSourceRect(float x, float y, float width, float height)
    {
        if (m_numTexture <= 0) return; // テクスチャがない場合は何もしない

        // 元画像の幅と高さを取得する
        float texW = static_cast<float>(GetTextureWidth(0));
        float texH = static_cast<float>(GetTextureHeight(0));

        // ピクセル座標を UV座標 (0.0f 〜 1.0f) に変換
        float u = x / texW;
        float v = y / texH;
        float uw = width / texW;
        float vh = height / texH;

        // ポリゴンのサイズ（中心からの距離）
        float halfW = m_size.x * 0.5f;
        float halfH = m_size.y * 0.5f;

        // UV座標を書き換えた新しい頂点データを作成
        SSimpleVertex vertices[] =
        {
            { Vector4(-halfW, -halfH, 0.0f, 1.0f), Vector2(u, v + vh) },       // 左下
            { Vector4(halfW, -halfH, 0.0f, 1.0f), Vector2(u + uw, v + vh) },  // 右下
            { Vector4(-halfW,  halfH, 0.0f, 1.0f), Vector2(u, v) },            // 左上
            { Vector4(halfW,  halfH, 0.0f, 1.0f), Vector2(u + uw, v) }        // 右上
        };
