...
namespace nsApp
{
	namespace nsCombat
	{
		namespace
		{
			...
			static_assert(
				_countof(sPartNameList_) == static_cast<size_t>(EnHitPart::Num),
			...
			HitBoxSet stInfectedHitBoxSet_;	//! 雑魚ゾンビの当たり判定。
			HitBoxSet stSurvivorHitBoxSet_;	//! サバイバーの当たり判定。
			 ...
			 */
			EnHitPart FindPartByName(const std::string& sName)
			{
				const int iNumPart = static_cast<int>(EnHitPart::Num);
				for (int i = 0; i < iNumPart; i++)
				{
					if (sName == sPartNameList_[i])
						return static_cast<EnHitPart>(i);
				}

				return EnHitPart::None;
			}
			 ...
			 * @param stSet 組み立て先。
			 */
			void BuildHumanDefault(HitBoxSet& stSet)
			{
				stSet.Clear();

				/* 上から順に、頭・胴(腕を含む)・脚。*/
				stSet.AddPartShape({ EnHitPart::Head, 148.0f, 178.0f, 20.0f });
				stSet.AddPartShape({ EnHitPart::Body,  75.0f, 148.0f, 38.0f });
				stSet.AddPartShape({ EnHitPart::Leg,    0.0f,  75.0f, 28.0f });

				stSet.SetDamageRate(EnHitPart::Head, 2.0f);
				stSet.SetDamageRate(EnHitPart::Body, 1.0f);
				stSet.SetDamageRate(EnHitPart::Leg, 0.7f);
			}
			 ...
			 */
			void OverwriteFromFile(nsData::ParameterFile& stFile, HitBoxSet& stSet)
			{
				...
				if (stFile.Enter(sDamageRateNodeName_))
				{
					...
					for (int i = 0; i < iNumPart; i++)
					{
						const EnHitPart enPart = static_cast<EnHitPart>(i);
				...
				for (int i = 0; i < iNumShape; i++)
				{
					...
					if (stShape.enPart_ == EnHitPart::None || stShape.fTop_ <= stShape.fBottom_ || stShape.fRadius_ <= 0.0f)
						continue;
				...
				float aRateList[static_cast<int>(EnHitPart::Num)] = {};
				for (int i = 0; i < static_cast<int>(EnHitPart::Num); i++)
					aRateList[i] = stSet.GetDamageRate(static_cast<EnHitPart>(i));

				stSet.Clear();

				for (const HitPartShape& stShape : vecShapeList)
					stSet.AddPartShape(stShape);

				for (int i = 0; i < static_cast<int>(EnHitPart::Num); i++)
					stSet.SetDamageRate(static_cast<EnHitPart>(i), aRateList[i]);
			}
			 ...
			 * @return 当たっていたらtrue。
			 */
			bool RayTestCylinder(const Vector3& vRayStart, const Vector3& vRayDirection, float fMaxDistance,
		...
		bool HitBoxSet::RayTest(const Vector3& vRayStart, const Vector3& vRayDirection, float fMaxDistance, const Vector3& vFootPosition, HitResult& stOutResult) const
		{
			...
			for (const HitPartShape& stShape : vecPartShapeList_)
			{
				...
				if (!RayTestCylinder(vRayStart, vRayDirection, fNearest, vFootPosition, fBottomY, fTopY, stShape.fRadius_, fDistance))
					continue;
