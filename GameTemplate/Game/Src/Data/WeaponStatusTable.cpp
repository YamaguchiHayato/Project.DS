...
namespace
{
	...
	void BuildHandgunDefault(WeaponStatusEntry& stEntry)
	{
		...
		stStatus.enSlot_ = nsApp::nsWeapon::EnWeaponSlot::Sub;
	 ...
	 * @param stEntry 組み立て先。
	 * @note  BuildHandgunDefault と同じく、JSONが読めないときのフォールバック。
	 */
	void BuildAssaultRifleDefault(WeaponStatusEntry& stEntry)
	{
		...
		stStatus.enSlot_ = nsApp::nsWeapon::EnWeaponSlot::Main;
	...
	void BuildDefaults()
	{
		BuildHandgunDefault(aEntryList_[static_cast<size_t>(nsApp::nsWeapon::EnWeaponType::Handgun)]);
		BuildAssaultRifleDefault(aEntryList_[static_cast<size_t>(nsApp::nsWeapon::EnWeaponType::AssaultRifle)]);
	...
	void OverwriteFromFile(const nsApp::nsData::ParameterFile& stFile, WeaponStatusEntry& stEntry)
	{
		...
		const std::string sSlot = stFile.GetString("slot", (stStatus.enSlot_ == nsApp::nsWeapon::EnWeaponSlot::Main) ? "Main" : "Sub");
		if (sSlot == "Main")
			stStatus.enSlot_ = nsApp::nsWeapon::EnWeaponSlot::Main;
		else if (sSlot == "Sub")
			stStatus.enSlot_ = nsApp::nsWeapon::EnWeaponSlot::Sub;
