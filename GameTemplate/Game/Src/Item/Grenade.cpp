...
namespace nsApp
{
	namespace nsItem
	{
		...
		void Grenade::Explode()
		{
			...
			vFxPos.y += 50.0f;
			nsEffect::HitEffect* pFx = NewGO<nsEffect::HitEffect>(0, "hitEffect");
			pFx->Setup(vFxPos, 60.0f, 260.0f, 0.35f);
			...
			for (nsActor::CommonEnemy* pEnemy : FindGOs<nsActor::CommonEnemy>("commonEnemy"))
			{
				...
				nsEffect::HitEffect* pHit = NewGO<nsEffect::HitEffect>(0, "hitEffect");
				pHit->Setup(vHitPos, 30.0f, 110.0f, 0.25f);
